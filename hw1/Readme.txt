...
가장 작은 Worker"에게 다음 작업을 보낸다. 추적하는 큐 크기는
  ...
  Worker가 받은 TASK 수)
로 계산한다. Worker는 작업을 큐에 받을 때, 큐에서 꺼내 처리를 시작할 때,
P2P로 작업을 주고받은 직후에 QUEUE(큐 크기 + 지금까지 받은 TASK 수)를
보고한다. 이 보고는 네트워크를 거쳐 늦게 도착하므로, 괄호 안(보냈지만
아직 도착하지 않은 작업)을 더하지 않으면 이미 가득 찬 Worker에게 계속
보내 거절이 쌓인다.
추적값이 10이 된 Worker에게는 보내지 않고, 보낼 수 있는 Worker가 없으면
해당 작업을 큐로 되돌리고 잠시 대기한다. 장애로 재할당된 작업도 같은
알고리즘을 쓰되, priorityQueue를 pendingQueue보다 먼저 소비하고 직전에
그 작업을 실패시킨 Worker는 후보에서 제외한다.
...
6) 이전 직후 보낸 쪽과 받은 쪽 모두 바뀐 큐 크기를 Master에 QUEUE로
   보고해서, Master의 분배 판단에 반영되게 함
