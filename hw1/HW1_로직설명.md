...
- 로그는 파일(`Master.txt`, `WorkerN.txt`)에만 순수 텍스트로 기록함 (`FileLogger.java`, 콘솔 출력 없음)
- 한 노드 안에서 여러 Thread가 같은 로그 파일에 쓰므로, 시각을 읽은 순서와 파일에 쓴 순서가 어긋날 수 있음 → `FileLogger`가 직전 줄보다 이른 시각은 직전 시각으로 올려 찍어서, 한 파일 안의 시각은 위에서 아래로 항상 같거나 커짐
...
3. **DISTRIB (동적 분배 루프, `runDispatchLoop`)**: `kvStore.isAllDone()`이 false인 동안 반복
   ...
   - 정상 분배 시: `clock.advance(NETWORK_DELAY)` → `kvStore.markDispatched()`로 재할당 여부 기록 (거절 응답이 바로 와도 원래 큐를 알 수 있도록 **전송 전에** 기록) → `scheduler.onDispatched()`로 그 Worker에게 보낸 누적 수 +1 → `ClientHandler.sendTask(key, index, value, isRetry, t)`로 TASK 전송
   - 로그: 신규 작업은 `DISTRIB INFO "Dispatching KV[..] -> WorkerN"`, 재할당 작업은 `DISTRIB SUCCESS "KV[..] reassigned to WorkerN (excluded WorkerM)."`
   - **Master가 보는 Worker 큐 크기 (`WorkloadScheduler`)** = Worker가 마지막으로 `QUEUE`로 보고한 크기 + (Master가 그 Worker에게 보낸 TASK 누적 수 − 그 보고에 실린 `recv` 값)
     - `recv`는 Worker가 Master에게서 받은 TASK 누적 수(거절한 것 포함, P2P로 받은 것 제외)
     - 보고는 통신 지연만큼 늦게 도착하므로, 괄호 안(보냈지만 아직 도착하지 않은 작업)을 더하지 않으면 이미 가득 찬 Worker에게 계속 보내게 됨. 예전에는 보고값으로 덮어써서 전송의 60% 이상이 큐 초과로 거절됐음
     - 이 값은 항상 실제 큐 크기 이상이므로(처리로 줄어든 건 늦게 알 뿐), Master 전송 때문에 큐가 넘치는 일은 없음. P2P로 받은 작업과 Master 작업이 같은 Worker에 동시에 몰릴 때만 드물게 거절이 생김
...
while (!shutdownRequested) {
    ...
    2) readyQueue에서 작업 하나만 꺼내 → QUEUE 보고 → 처리 (processTask)
...
세부 동작:
- **TASK 수신**: `retry=true`면 재할당 작업으로 보고 `retryReceived` 증가, `RECV INFO "Received PRIORITY task (reassigned)"` 로그. `readyQueue.offer()` 시도 → 꽉 찼으면(10/10) 즉시 거절하고 `RECV FAIL` 로그 + `RESULT=REJECTED` 응답(`queueRejected` 증가), 성공하면 70% 초과 WARN 체크 + `QUEUE` 보고 (`size`와 함께 Master에게서 받은 TASK 누적 수 `recv`를 실음)
...
- **P2PServer**: 다른 Worker의 `P2P_QUERY`(내 큐 크기 응답), `P2P_TRANSFER`(작업 수신 후 `p2pReceived` 누적, 1건 이상 받았으면 바로 Master에 `QUEUE` 보고, ACK 응답) 처리. 받은 작업은 앞에서부터 큐에 넣다가 하나라도 못 넣으면 멈춤 — 중간 것을 건너뛰고 뒤의 것을 받으면, 보낸 쪽 롤백 계산과 어긋나 작업 유실/중복이 생기기 때문
...
| Master→Worker | `TASK` | key, index(로그용 순번), value, retry(재할당 작업이면 true), clock |
...
| Worker↔Worker | `P2P_TRANSFER` | fromId, keys/indexes/values(각각 `;`구분), clock |
...
로그는 `.txt` 파일에만 순수 텍스트로 남고, 한 파일 안의 시각은 항상 위→아래로 같거나 커짐 (1장 참고).
